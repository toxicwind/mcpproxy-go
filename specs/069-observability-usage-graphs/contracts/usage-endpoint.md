...
- `blocked` counts policy-prevented attempts (persisted as blocked `policy_decision` records, not executed tool_calls). A blocked attempt never ran, so it is **not** included in `calls` and contributes no latency/bytes — it only increments `blocked` and `last_used`. The timeline mirrors the tray glance row-for-row: it counts executed tool calls, blocked attempts (as calls and errors), and mcpproxy's own internal calls (`retrieve_tools`, `describe_tool`; plus any failed non-management built-in, `code_execution` included), and excludes `call_tool_*` echo records, management built-ins (`upstream_servers`, `quarantine_security`), successful `code_execution` wrappers, and concurrency-rejected sheds. A successful `code_execution` is represented by its sub-calls — they are `tool_call` records that count on their own — so counting the wrapper too would double the script under a name no upstream owns; a **failed** one has no sub-calls to speak for it and still counts. *(Amended by PR #1016 — the timeline previously tracked executed upstream tool_calls only, which left the histogram empty under retrieve_tools/code_execution-heavy traffic. Amended again by `fix/tray-glance-compact` — successful `code_execution` wrappers left the set once their sub-calls became rows of their own; `usageAdmissionVersion` went to 4 so pre-change snapshots rebuild. The glance↔timeline parity is pinned by usage_glance_parity_test.go with the Go and Swift admission sets kept in lockstep.)*
